...
export class SwitchboardDecimal {
  ...
  public static fromBig(big: Big): SwitchboardDecimal {
    ...
    if (big.sub(result.toBig()).abs().gt(new Big(0.000_05))) {
      throw new Error(
...
export class ProgramStateAccount {
  ...
  public constructor(params: AccountParams) {
    ...
    if (
      ...
      params.publicKey !== undefined &&
      !params.publicKey.equals(params.keypair.publicKey)
    ) {
      throw new Error(
...
export enum SwitchboardPermissionValue {
  PERMIT_ORACLE_HEARTBEAT = Math.trunc(1),
...
export class PermissionAccount {
  ...
  async isPermissionEnabled(
    ...
    const { permissions } = await this.loadData();
...
export function signTransactions(
  ...
  for (const transaction of transactions) {
    ...
    const sigsNeeded = transaction.instructions.flatMap((instruction) => {
      // eslint-disable-next-line @typescript-eslint/no-shadow
